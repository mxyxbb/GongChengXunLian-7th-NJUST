...
extern uint8_t Mem[SMSBCL_PRESENT_CURRENT_H-SMSBCL_PRESENT_POSITION_L+1];
extern int Err;
...
SHELL_EXPORT_CMD(SHELL_CMD_PERMISSION(0)|SHELL_CMD_TYPE(SHELL_TYPE_CMD_FUNC), wpe, WritePosEx, WritePosEx(id,po,speed,acc));
...
int WriteSpe(uint8_t ID, int16_t Speed, uint8_t ACC)
{
	...
	syncWrite(&ID, 1, SMSBCL_GOAL_SPEED_L, bBuf, 2);
	End = 1;
	return 1;
}
